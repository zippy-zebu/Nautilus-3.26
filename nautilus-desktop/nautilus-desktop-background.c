...
nautilus_desktop_background_set_up_widget (NautilusDesktopBackground *self)
{
    ...
    in_fade = fade_to_surface (self, window, self->details->background_surface);
...
background_changed_cb (NautilusDesktopBackground *self)
{
    ...
    nautilus_canvas_container_request_update_all (NAUTILUS_CANVAS_CONTAINER (self->details->widget));
...
queue_background_change (NautilusDesktopBackground *self)
{
    ...
    self->details->change_idle_id = g_idle_add ((GSourceFunc) background_changed_cb, self);
}
                   ...
                   gpointer user_data)
{
    ...
    self->details->screen_size_handler = g_signal_connect (screen, "size-changed",
    ...
    self->details->screen_monitors_handler = g_signal_connect (screen, "monitors-changed",
                                                               G_CALLBACK (screen_size_changed), self);
                                     ...
                                     gpointer   user_data)
{
     ...
     */
    g_idle_add ((GSourceFunc) background_change_event_idle_cb, g_object_ref (self));
