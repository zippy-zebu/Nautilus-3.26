...
unrealize (GtkWidget *widget)
{
    ...
    if (details->monitors_changed_id != 0)
    {
        g_signal_handler_disconnect (gtk_window_get_screen (GTK_WINDOW (window)),
                                     details->monitors_changed_id);
        details->monitors_changed_id = 0;
    }
...
realize (GtkWidget *widget)
{
    ...
    details->monitors_changed_id =
        g_signal_connect (gtk_window_get_screen (GTK_WINDOW (window)), "monitors-changed",
                          G_CALLBACK (nautilus_desktop_window_screen_size_changed), window);
}
