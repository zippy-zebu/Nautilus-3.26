...
search_engine_start_real (NautilusSearchEngine *engine)
{
    ...
    priv->providers_running++;
    nautilus_search_provider_start (NAUTILUS_SEARCH_PROVIDER (priv->locate));
...
nautilus_search_engine_init (NautilusSearchEngine *engine)
{
    ...
    priv->locate = nautilus_search_engine_locate_new ();
    connect_provider_signals (engine, NAUTILUS_SEARCH_PROVIDER (priv->locate));
}
